...
def send_match_notification_email(recipient_email, case_id, case_name, location_details, image_bytes, reporter_info):
    ...
    # --- CORRECTED MAP LINK ---
    maps_link = f"https://www.google.com/maps?q={location_details['lat']},{location_details['lon']}"
...
def analyze_photo():
    try:
        ...
        # OPTIMIZED: Reduced width to 500 for faster real-time processing
        max_width = 500
            ...
            for name in individual_names:
                for case in active_cases:
                    if case['name'] == name:
                        ...
                        # Send email ONLY if single photo upload
                        if source == 'upload':
...
# --- DELETE ROUTE ---
@app.route('/api/delete_case/<case_id>', methods=['POST'])
@login_required
def delete_case(case_id):
    try:
        ...
        if case.get('createdBy') != current_user.id:
            return jsonify({'success': False, 'message': 'Unauthorized.'}), 403

        cases_collection.delete_one({'_id': ObjectId(case_id)})
        sightings_collection.delete_many({'caseId': case_id})
